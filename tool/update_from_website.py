...
# 新規感染者数
new = int(mojimoji.zen_to_han(re.search(r"新規感染者数　(.+?)人", summary).group(1)))
# 陽性患者数
total = int(mojimoji.zen_to_han(re.search(r"累計　(.+?)人", summary).group(1)))
# 入院中
hospitalized = int(mojimoji.zen_to_han(
    re.search(r"入院中　(.+?)人", summary).group(1)))
# 重症
...
# 自宅療養
home = int(mojimoji.zen_to_han(re.search(r"自宅療養または入院等調整中　(.+?)人", summary).group(1)))
# 死亡
death = int(mojimoji.zen_to_han(re.search(r"死亡者数　(.+?)人", summary).group(1)))
# 退院
discharged = int(mojimoji.zen_to_han(re.search(r"退院者数　(.+?)人", summary).group(1)))
